# Privacy Policy

**Last Updated:** December 13, 2025

## 1. Introduction

Shoppy+ ("we," "us," "our," or "Company") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and otherwise process personal information in connection with our mobile application and services (collectively, the "Service").

Please read this Privacy Policy carefully. By accessing and using Shoppy+, you acknowledge that you have read, understood, and agree to be bound by all the terms of this Privacy Policy.

If you do not agree with our policies and practices, please do not use our Service.

---

## 2. Information We Collect

### 2.1 Information You Provide Directly

We collect the following personal information when you create an account or use our Service:

#### **Account Registration Information**
- **Full Name** - Required for account identification
- **Email Address** - Required for account communication and authentication
- **Phone Number** - Optional, used for customer support and notifications

#### **How We Collect This Information**
- During account registration/signup
- When updating your profile
- When contacting customer support

### 2.2 Automatically Collected Information

We may automatically collect certain information about your device and usage:

- Device type and operating system
- App version and build number
- Crash reports and error logs
- User interaction data (features used, time spent)
- IP address and device identifiers
- Push notification tokens

### 2.3 Information from Third Parties

We may receive information from:
- Authentication providers (Supabase)
- Payment processors (if applicable)
- Analytics services
- Email service providers (Resend)

---

## 3. How We Use Your Information

We use the personal information we collect for the following purposes:

### 3.1 Core Service Delivery
- Creating and managing your account
- Contacting you about account updates and support requests
- Maintaining service functionality

### 3.2 Communication
- Sending transactional emails (order confirmations, account updates)
- Responding to customer support requests
- Sending important service announcements
- Notifying about account changes or security issues

### 3.3 Account Management
- Verifying your identity
- Managing account settings and preferences
- Processing account deletion requests
- Maintaining account security

### 3.4 Improving Our Service
- Analyzing app usage and performance
- Identifying and fixing technical issues
- Understanding user preferences
- Developing new features and improvements

### 3.5 Legal & Safety
- Complying with legal obligations
- Enforcing our Terms of Service
- Protecting against fraud and security threats
- Protecting the rights and safety of our users

---

## 4. Data Security

### 4.1 Security Measures

We implement appropriate technical and organizational security measures to protect your personal information, including:

- Encrypted data transmission (SSL/TLS)
- Secure authentication mechanisms
- Regular security assessments
- Limited access to personal data
- Secure backend infrastructure (Supabase)

### 4.2 Data Storage

Your personal information is stored securely in our Supabase database with:
- Encryption at rest
- Regular backups
- Access controls and authentication
- Monitoring and intrusion detection

### 4.3 No Guaranteed Security

While we implement security measures, no method of transmission over the Internet or electronic storage is 100% secure. We cannot guarantee absolute security of your information.

---

## 5. Data Retention

### 5.1 Retention Period

We retain your personal information for as long as your account is active and as long as necessary to provide you with our Service.

### 5.2 After Account Deletion

When you request account deletion:
- We process your deletion request within 48 hours
- All personal data (name, email, phone) is permanently deleted
- You will receive a confirmation email
- Some data may be retained if required by law

### 5.3 Backup Data

Deleted information may persist in our backups for a reasonable period but will not be accessible or used.

---

## 6. Sharing of Information

### 6.1 When We Share Your Information

We may share your personal information in the following circumstances:

#### **Service Providers**
- Payment processors (for transactions)
- Email service providers (Resend - for communications)
- Authentication services (Supabase)
- Analytics providers
- Customer support tools

#### **Legal Requirements**
- When required by law or regulation
- To comply with legal processes
- To protect our legal rights
- In response to government requests

#### **Business Transfers**
- In case of merger, acquisition, or bankruptcy
- Personal information may be transferred as part of business assets

### 6.2 Third-Party Links

Our Service may contain links to third-party websites. We are not responsible for their privacy practices. Please review their privacy policies independently.

### 6.3 No Sale of Data

**We do not sell your personal information to third parties.**

---

## 7. Your Privacy Rights

### 7.1 Access Your Information

You have the right to:
- Access the personal information we hold about you
- Request a copy of your data
- Update or correct your information
- Contact us at: shoppy.dot.plus@gmail.com

### 7.2 Delete Your Account

You can request account deletion anytime through the app:
1. Go to **Profile** → **Account**
2. Tap **Delete Account**
3. Review the information and confirm
4. We process deletion within 48 hours

### 7.3 Data Portability

You may request your data in a portable format. Contact us at: shoppy.dot.plus@gmail.com

### 7.4 Opt-Out of Communications

You can manage notification preferences in:
- **Profile** → **Settings** → **Notifications**

---

## 8. Children's Privacy

### 8.1 Age Restriction

Shoppy is not intended for children under 13 years of age. We do not knowingly collect personal information from children under 13.

### 8.2 Parental Consent

If we discover we've collected information from a child under 13 without parental consent, we will delete it immediately.

### 8.3 Contact

If you believe we've collected information from a child, please contact: shoppy.dot.plus@gmail.com

---

## 9. International Data Transfers

### 9.1 Data Location

Your information may be transferred to, stored in, and processed in countries other than your country of residence, including countries that may not have the same data protection laws.

### 9.2 Consent

By using Shoppy+, you consent to the transfer of your information to countries outside your country of residence.

---

## 10. Changes to This Privacy Policy

### 10.1 Updates

We may update this Privacy Policy to reflect changes in our practices, technology, or legal requirements.

### 10.2 Notification

We will notify you of significant changes by:
- Updating the "Last Updated" date
- Sending an email notification
- Displaying a prominent notice in the app

### 10.3 Continued Use

Your continued use of Shoppy+ following changes constitutes your acceptance of the updated Privacy Policy.

---

## 11. Contact Us

### 11.1 Privacy Inquiries

For questions about this Privacy Policy or our privacy practices, please contact:

**Email:** shoppy.dot.plus@gmail.com  

### 11.2 Response Time

We will respond to privacy inquiries within 30 days.

---

## 12. Specific Disclosures by Jurisdiction

### 12.1 GDPR (European Users)

If you are located in the EU/EEA, you have additional rights under GDPR:
- Right to be informed
- Right of access
- Right to rectification
- Right to erasure ("right to be forgotten")
- Right to restrict processing
- Right to data portability
- Right to object
- Rights related to automated decision-making

**Legal Basis for Processing:**
- **Consent** - Account creation and profile data
- **Contract** - Delivery and service provision
- **Legal Obligation** - Data retention and compliance
- **Legitimate Interests** - Service improvement and security

### 12.2 CCPA (California Users)

If you are a California resident, you have the right to:
- Know what personal information is collected
- Know whether your data is sold or disclosed
- Delete personal information
- Opt-out of the sale of personal information
- Non-discrimination for exercising your rights

To exercise these rights, email: shoppy.dot.plus@gmail.com

---

## 13. Data Processing Terms

### 13.1 Service Providers

We use the following services that process your data:

| Service | Purpose | Data Processed |
|---------|---------|-----------------|
| **Supabase** | Database & Authentication | All personal data |
| **Resend** | Email Communications | Email address, name |
| **Expo** | App Distribution | Device info, usage |

### 13.2 Data Processing Agreements

We have Data Processing Agreements with all third-party service providers.

---

## 14. Disclaimer

This Privacy Policy is provided for informational purposes. While we strive for accuracy, we make no warranties regarding completeness or accuracy.

---

## Version History

| Version | Date | Changes |
|---------|------|---------|
| 1.0 | 2025-12-13 | Initial Privacy Policy for Play Store |

---

**Status:** Active  
**Document Owner:** Admin Team  
**Last Reviewed:** December 13, 2025

---

**© 2025 Shoppy+. All rights reserved.**
